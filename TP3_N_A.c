...
#include <stdio.h>
#include <stdlib.h>
int combien(int age[], int nbPers, int limit )					//int fonction combien, integer du tableau age, entier du nbpersonne et de la limite
...
int min (int nbcafe[], int nbPers, int borne)					// consomation minimal de cafe fonctiion avec return
  {
  	int n = 0;
  	int i = 0;
  	for( i = 0; i < nbPers; i++)
...
int minb (int age[], int nbPers, int borneb)				// fonction avec return age min des employes integer
  {
  	int n = 0;
  	int i = 0;
  	for(i = 0; i < nbPers; i++)
...
int nbDivi (int nombre) {						// fonction avec return pour avant le main pour les compter et combien le nombre a de diviseur
...
void diviseur (int nb){							// fonction pour compter et affiche le nombre de diviseur
	int increment, compteur = 0;
	printf ("2-");
    printf ("Les diviseurs de %3d sont : ",nb);
 ...
 float moyenne(int tableau[], int nbElem )  		// fonction pour la moyenne de l'age avec un return en float et cons min cafe 
 {  int i;
    float somme = 0.0;
    for( i = 0; i < nbElem; i++) 
         somme += tableau[i];
  ...
  void afficher(char poste[], int nbcafe[], int age[], int nbPers, char quand[] )  // fonction pour afficher les tableau avant triage et apres
    {
    	int i=0;											// initialisation du i 
		printf("3 | 7- Contenu des 3 tableaux %s :\n", quand); // aff contenue des 3 tableau
		printf("nbcafe  age   	poste\n");					// aff du nbcafe afe et poste au debut
    	for( i = 0; i < nbPers; i++)						// parcourir les personnes 
    	{
		  printf("%d)%d  %5d ",i, nbcafe[i], age[i]);		// entier tableau age et cafe
		  switch( poste[i])									// switch du poste
		  {
		  	case 'A' : printf("     Analyste"); break ;		// aff analyste
		  	case 'P' : printf("     Programmeur"); break ;	// aff programmeur
		  	case 'O' : printf("     Operateur"); break ;	// aff operateur
		  	default  : printf("     autre") ;				// no case 
		  };
		  printf("\n");										// passer une ligne 
	    }
		printf("\n\n")  ;									// Passer deux ligne
...
void trier(char poste[], int nbcafe[],int age[], int nbPers) // Trier le tableau pas rapport a l'age 
    {   int i = 0;											// Commencer par le poste, nbcafe , l'age et le nombre de personne 
    	for(i = 0; i < nbPers-1; i++)
    	{
    		...
    		if (indMin != i)      
    		{
    			...
    			tempo = age[i];								// initialisation de l'age 
    			age[i]= age[indMin];
    			age[indMin]= tempo;
...
int main() {													// fonction principale 
	...
	printf("4- L age moyenne des employes sont : %.2f \n", moyenne(age, nbPers));	// age moyen employe en float
	printf("4- La consommation moyen de cafes des employes sont : %.1f cafes\n\n",moyenne(nbcafe, nbPers));	// consommaiton moyenne de cafe en float
	printf("5- Il y a %d pers. qui ont plus de 25 ans \n", combien(age, nbPers, 25));	// employe ayant 25+
	printf("5- Il y a %d pers. qui consomme plus de 2 cafes par jour \n\n", combien(nbcafe, nbPers, 2)); // aff employe consomant 2+ /jour
	printf("6- Consommation la plus petite de cafe est : %d \n", min(nbcafe, nbPers,40 ));	// affichage consommation minimal cafe
	printf("6- Age minimal de tous les employes : %d \n\n", minb(age, nbPers, 40)); // affichage age minimal employe
	...
	return 0;													// retour a zero de la fonction main 
...
--------------------------------
Process exited after 0.05427 seconds with return value 0
